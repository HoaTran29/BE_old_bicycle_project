...
- The order lifecycle is clearer now that seller completion only moves an order to `awaiting_buyer_confirmation`, and buyer/admin confirmation is the step that actually releases funds and closes the transaction.
...
| `F-008` | Deposit & Order | Must | `Partial` | Buyers can create orders with `partial/full` upfront intent, sellers can accept them, buyers can request payment instructions, and the order lifecycle now includes `awaiting_buyer_confirmation` plus `confirm-received` before funds are released and the product becomes `sold`. | Remaining payment phase, auto-release/timeout policy, transaction logging, richer dispute handling, and integration-level tests are still missing. |
...
| `F-011` | Admin Dashboard | Must | `Partial` | Dashboard stats endpoint exists. Admin user management now covers list/filter/detail/status change/reset password/activity, product moderation now has filter + approve/hide shortcuts, admin CRUD now exists for brands, categories, brake types, and frame materials, and admin refund review now has a paged listing endpoint plus review actions for FE dispute screens. Report processing also records which admin handled a case plus when it was processed. | Dispute resolution breadth is better but still not full SRS depth, richer analytics are still thin, and the remaining technical master-data modules such as groupset/size chart are still missing. |
...
| `F-016` | Online Payment | Could | `Partial` | A usable phase-1 flow now exists: create payment request, generate transfer instructions and QR, receive SePay-style webhook confirmation, persist payment records, support refund request/admin review, and expose admin refund list/review APIs for FE. Non-mock SePay order creation is now implemented via configurable API integration, webhook validation accepts both secret-header and legacy API-key style callbacks, and the current MBBank environment has now been live-verified through ngrok-routed IPN using the direct-transfer fallback path. Webhook-only regression now also covers malformed JSON, missing code, and snake_case payload variants. | Remaining-payment phase, payout/release automation, post-completion dispute handling, full refund automation, and richer bank/gateway-specific payment depth are still missing. |
...
8. Admin dispute handling is also stronger now that refund requests can be listed and reviewed through a dedicated admin refund API instead of relying on partial order-only views.
9. The order lifecycle is closer to an escrow model because seller completion no longer releases funds immediately; buyer/admin confirmation is now the final release step.
10. The largest remaining MVP quality gaps are now broader integration coverage, review/reply depth, groupset/size-chart management, and richer payment depth beyond the initial SePay live path.
11. Video/media support remains a full-SRS gap, but it is no longer a near-term MVP milestone.

...
| `F-004` | Advanced Filter | Must | `Partial` | Technical filters now cover brand, category, brake, frame material, condition, price, province, frame size, wheel size, groupset, and verified status. | Video-aware filtering is still missing because the backend has no product video model/storage flow yet. |
| `F-005` | Bike Detail View | Must | `Partial` | Product detail now returns listing data with images, real verified badge state, and a public inspection summary/report block when inspection data exists. | Seller trust depth, richer media support, and some remaining SRS detail fields are still incomplete. |
...
- Finish the remaining Product/Inspection gaps: video/media support, repository/integration coverage for search filtering, and any admin moderation rules still tied to listing state transitions.
